...
- Approach 1:
  - priority
null  - left most of right - O(depth of right)
null  - right - O(1)
null  - parent - O(1)
      - current.parent.right = current return null
      - parent.value < currentNode.value return null;
      - return inputNode.parent - WRONG!
             20
           /    \
          9      25
           \    /  \
...
BinarySearchTree.prototype.findInOrderSuccessor = function(inputNode) {
  // leftmost of right
  let right = inputNode.right;
  while(right.left !== null) {
     right = right.left;
  }
  if(right !== null) return right;
  // parent
  let parentTraversal = inputNode.parent;
  while(
    parentTraversal !== null 
    && parentTraversal.key < inputNode.key
  ) {
